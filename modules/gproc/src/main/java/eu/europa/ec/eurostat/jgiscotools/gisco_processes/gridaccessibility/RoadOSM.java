...
public class RoadOSM {
	...
	public static Collection<Feature> get(String costAttribute) {
		Filter fil = null;
		try {
			fil = CQL.toFilter("(NOT fclass='bridleway' AND NOT fclass='cycleway' AND NOT fclass='footway' AND NOT fclass='path' AND NOT fclass='steps' AND NOT fclass='track' AND NOT fclass='track_grade1' AND NOT fclass='track_grade2' AND NOT fclass='track_grade3' AND NOT fclass='track_grade4' AND NOT fclass='track_grade5')");
	...
	private static double getSpeedKmH(String maxspeed, String fclass) {
		...
		if("trunk".equals(fclass)) return 70.0;
		if("trunk_link".equals(fclass)) return 40.0;
		if("primary".equals(fclass)) return 90.0;
		if("primary_link".equals(fclass)) return 40.0;
		if("secondary".equals(fclass)) return 80.0;
		if("secondary_link".equals(fclass)) return 40.0;
		if("tertiary".equals(fclass)) return 70.0;
		if("tertiary_link".equals(fclass)) return 40.0;

		if("residential".equals(fclass)) return 40.0;
		if("service".equals(fclass)) return 30.0;
		if("living_street".equals(fclass)) return 30.0;
		if("pedestrian".equals(fclass)) return 5.0;

		if("unclassified".equals(fclass)) return 50.0;
		if("unknown".equals(fclass)) return 50.0;
