...
public class TesselationGeneralisation {
	...
	public static Collection<Feature> runGeneralisation(Collection<Feature> units, HashMap<String, Collection<Point>> points, final TesselationGeneralisationSpecification specs, boolean parallel, int roundNb, int maxCoordinatesNumber, int objMaxCoordinateNumber) {
		...
		for(int i=1; i<=roundNb; i++) {
			...
			}, parallel, maxCoordinatesNumber, objMaxCoordinateNumber, true, true, Partition.GeomType.ONLY_AREAS, 0.02);
	...
	//
	public static HashMap<String,Collection<Point>> indexPoints(Collection<Feature> pts, String idProp) {
		HashMap<String,Collection<Point>> index = new HashMap<String,Collection<Point>>();
		for(Feature f : pts) {
			String id = f.getAttribute(idProp).toString();
			if(id == null) {
				LOGGER.warn("Could not find id "+idProp+" for points.");
				return null;
			}
